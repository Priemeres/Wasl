...
- Windows x64: `npm run dist:win` creates `Wasl-0.5.0-win-x64.exe` (NSIS installer) and `.zip` (portable folder).
- Linux x64: `npm run dist:linux` creates `Wasl-0.5.0-linux-x64.AppImage` and `.tar.gz`.
...
- `npm run source` produces `Wasl-0.5.0-source.zip` with all source, docs, lockfile, license, and workflow.
...
3. To prepare a release, ensure the version in package.json and package-lock.json matches the version tag, then push a tag such as `v0.5.0`.
