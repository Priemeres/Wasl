...
The `desktop` folder is a standalone repository. Use its contents (or extract the source ZIP) as the GitHub repository root. Do not upload node_modules, local databases, release binaries into Git source, tokens, or the former website project.

## Native builds

Node 24 LTS, npm, and Git are recommended. Install dependencies with `npm ci`, then run `npm run check` and `npm test`.

- Windows x64: `npm run dist:win` creates `Wasl-0.3.0-win-x64.exe` (NSIS installer) and `.zip` (portable folder).
- Linux x64: `npm run dist:linux` creates `Wasl-0.3.0-linux-x64.AppImage` and `.tar.gz`.
...
- `npm run source` produces `Wasl-0.3.0-source.zip` with all source, docs, lockfile, license, and workflow.
...
3. To prepare a release, ensure the version in package.json and package-lock.json matches the version tag, then push a tag such as `v0.3.0`.
