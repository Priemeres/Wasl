...
function Chart({ data, period }: {
    ...
    <AreaChart data={data} margin={{ top: 10, left: -24, right: 15, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 5" vertical={false} stroke="#1a7f57" strokeOpacity={0.18}/>
      <XAxis dataKey="time" tickFormatter={value => new Date(value).toLocaleString(locale(), period === '7d' ? { day: 'numeric', month: 'short' } : { hour: '2-digit', minute: '2-digit' })} tickLine={false} axisLine={false} minTickGap={30} tick={{ fontSize: 12, fill: '#566f61' }} dy={8}/>
      <YAxis allowDecimals={false} tickFormatter={number} tickLine={false} axisLine={false} tick={{ fontSize: 12, fill: '#566f61' }}/>
      <Tooltip labelFormatter={label => new Date(Number(label)).toLocaleString(locale())} formatter={value => [number(Number(value)), t('قراءة')]} cursor={{ stroke: '#1a7f57', strokeOpacity: 0.5 }} contentStyle={{ direction: document.documentElement.dir as 'rtl' | 'ltr', borderRadius: 3, border: '1px solid #7db89f', background: '#ffffff', color: '#10261b', fontFamily: 'Noto Sans Arabic', fontSize: 13, boxShadow: '0 10px 24px -14px rgba(16,38,27,.35)' }} itemStyle={{ color: '#125f42' }} labelStyle={{ color: '#566f61' }}/>
      <Area type="monotone" dataKey="messages" stroke="#1a7f57" strokeWidth={2} fill="none" isAnimationActive={false}/>
    </AreaChart>
...
export default function App() {
    ...
    <div className="app">
      ...
      <main>
        {page !== 'ide' && <div className={`page-heading ${page === 'overview' || page === 'guide' ? 'scaled' : ''}`}><div><h1>{page === 'overview' ? t('نظرة على مساحة عملك') : pages.find(p => p.id === page)?.name}</h1><p>{page === 'overview' ? t('أجهزتك وبياناتك ومشاريعك، في مكان واحد.') : page === 'devices' ? t('أضف أجهزتك وتابع أحدث قراءاتها.') : page === 'projects' ? t('من أول نموذج تجريبي إلى خط الإنتاج.') : page === 'telemetry' ? t('تابع الإشارات القادمة من أجهزتك.') : page === 'alerts' ? t('الأجهزة التي تحتاج إلى انتباهك.') : page === 'settings' ? t('إعدادات الاستقبال والبيانات المحلية.') : t('تعرّف على طريقة توصيل جهازك ببرنامج وصل.')}</p></div>
          {page !== 'guide' && <div className="heading-actions"><button className="button" onClick={exportDevices} disabled={demo} title={demo ? t('انتقل إلى أجهزتي لتصدير الأجهزة الفعلية') : t('حفظ ملف CSV')}><ArrowDownToLine size={16}/>{t("تصدير")}</button><button className="button primary" onClick={() => { setFormError(''); setModal('add'); }}><Plus size={18}/>{t("إضافة جهاز")}</button></div>}
        </div>}
        {page !== 'ide' && page !== 'guide' && <div className="mode-bar"><div className="segmented" aria-label={t("مصدر البيانات")}><button className={!demo ? 'selected' : ''} aria-pressed={!demo} onClick={() => mode(false)}><Cpu size={15}/>{t("أجهزتي")}</button><button className={demo ? 'selected' : ''} aria-pressed={demo} onClick={() => mode(true)}><Activity size={15}/>{t("بيانات تجريبية")}</button></div><span>{demo ? t('بيانات توضيحية للتجربة، وليست قراءات من أجهزة حقيقية.') : t('تُحفظ البيانات محلياً ويُحدَّث العرض كل ٣ ثوانٍ.')}</span></div>}
        ...
        {page === 'guide' && <Guide runtime={runtime} copy={copy} navigate={navigate} onAddDevice={() => { setFormError(''); setModal('add'); }} onMyDevices={() => { mode(false); navigate('devices'); }}/>}
